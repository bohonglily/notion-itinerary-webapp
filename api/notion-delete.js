...
import { NotionDeleteHandler } from '../src/serverless/handlers/notion-delete-handler.js';
import { PlatformAdapter } from '../src/serverless/core/platform-adapter.js';

// 創建 Handler 實例
const handler = new NotionDeleteHandler();

export default async function notionDelete(req, res) {
  try {
    // 轉換 Vercel 請求為統一格式
    const request = PlatformAdapter.fromVercelRequest(req);
    const context = PlatformAdapter.createContext('notion-delete');
