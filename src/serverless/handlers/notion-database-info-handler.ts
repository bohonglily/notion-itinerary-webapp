...
export class NotionDatabaseInfoHandler extends BaseHandler {
  ...
  async handle(request: ServerlessRequest, context: ServerlessContext): Promise<ServerlessResponse> {
    ...
    try {
      ...
      const result = await this.notionService.getDatabaseInfo(databaseId);

      return this.createSuccessResponse(result);

    } catch (error: any) {
      console.error('Notion database info error:', error);
      return this.createErrorResponse(
        500,
        'Failed to get Notion database info',
        error.message
      );
    }
  }
