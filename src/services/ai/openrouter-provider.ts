...
export class OpenRouterProvider extends AbstractAIProvider {
  ...
  async generateDescription(placeName: string, context?: string): Promise<string> {
    ...
    try {
      console.log('[OpenRouter] 呼叫參數:', {
        endpoint: this.endpoint,
        apiKey: this.apiKey?.slice(0, 8) + '...隱藏',
      ...
      console.log('[OpenRouter] 回傳:', response.data);
      const description = response.data.choices?.[0]?.message?.content?.trim();
      return description || '';
    } catch (error: unknown) {
      if (axios.isAxiosError(error) && error.response) {
        console.error('[OpenRouter] API 錯誤:', error.response.status, error.response.data);
      } else {
        console.error('[OpenRouter] 其他錯誤:', error);
      }
      return '';
    }
  }
  ...
  async generateBulkDescriptionsWithPrompt(items: ItemToProcess[], prompt: string): Promise<GeneratedDescription[]> {
    ...
    try {
      ...
      console.log('OpenRouter Raw Response:', rawResponse); // Log raw response
      if (!rawResponse) {
        throw new Error('OpenRouter API returned empty response for bulk descriptions.');
      }

      // OpenRouter might return a stringified JSON, parse it
      const generatedData: GeneratedDescription[] = JSON.parse(rawResponse);
      return generatedData;
    } catch (error: unknown) {
      if (axios.isAxiosError(error) && error.response) {
        console.error('[OpenRouter] Bulk API 錯誤:', error.response.status, error.response.data);
      } else {
        console.error('[OpenRouter] Bulk 其他錯誤:', error);
      }
      throw new Error(`OpenRouter Bulk API failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
